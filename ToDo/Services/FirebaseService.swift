...
class FirebaseService {
    ...
    func fetchTasksForList(listId: String, completion: @escaping (Result<[Task], FirebaseError>) -> Void) {
        ...
        // IMPORTANT: Values need quotes *escaped within the string* for Firebase REST
        let queryItems = [
            URLQueryItem(name: "orderBy", value: "\"listId\""), // Order by the child key "listId"
            URLQueryItem(name: "equalTo", value: "\"\(listId)\"") // Value must be a JSON string (quotes included)
        ]
        //                            ^-- Start quote for JSON string
        //                                ^-- End quote for JSON string

        guard let url = buildURL(for: "tasks", queryItems: queryItems) else {
            // Construct the query string manually for the error message to be clearer
            let queryStringForError = "tasks?orderBy=\"listId\"&equalTo=\"\(listId)\""
            completion(.failure(.invalidURL(queryStringForError)))
            return
        }

        print("Fetching tasks with URL: \(url.absoluteString)") // Add logging to see the constructed URL
        ...
        performRequest(url: url, method: "GET") { result in
            ...
            case .success(let data):
                ...
                guard let dictionary = try? JSONSerialization.jsonObject(with: data) as? [String: [String: Any]] else {
                    ...
                    // Provide more context in the decoding error
                    let responseString = String(data: data, encoding: .utf8) ?? "Non-UTF8 data"
                    completion(.failure(.decodingError(NSError(domain: "FirebaseService", code: 2, userInfo: [NSLocalizedDescriptionKey: "Expected dictionary of Tasks, received: \(responseString)"]))))
                    return
                }
